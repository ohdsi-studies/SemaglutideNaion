...
excludedCovariateConceptSet <- ROhdsiWebApi::getConceptSetDefinition(
  conceptSetId = 7150,
  baseUrl = baseUrl
)
# Extract the list of concepts
excludedCovariateConcepts <- lapply(
  excludedCovariateConceptSet$expression$items, 
  function(x) {
    x$concept
  })

excludedCovariateConcepts <- do.call(rbind, lapply(excludedCovariateConcepts, as.data.frame))
names(excludedCovariateConcepts) <- SqlRender::snakeCaseToCamelCase(names(excludedCovariateConcepts))
CohortGenerator::writeCsv(
  x = excludedCovariateConcepts,
  file = file.path("inst", "excludedCovariateConcepts.csv"),
  warnOnFileNameCaseMismatch = F
)